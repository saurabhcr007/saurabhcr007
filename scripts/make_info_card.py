...
ROWS = [
    ...
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Cloud", "AWS (EC2, EKS, S3, IAM, Lambda)"),
    ("kv", "IaC", "Terraform, Ansible"),
    ("kv", "Containers", "Docker, Kubernetes, Helm, ArgoCD"),
    ("kv", "CI/CD", "GitHub Actions, Jenkins"),
    ("kv", "Monitoring", "Prometheus, Grafana, Loki"),
    ("gap",),
    ("sec", "Certs"),
    ("bul", "AWS Observability — Completed"),
    ("bul", "DevOps on AWS — Completed"),
    ("bul", "Linux LFS101 — Completed"),
    ("bul", "AWS SAA-C03 — In Progress"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">saurabh@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">saurabh</tspan><tspan fill="{MUTED}">@</tspan>'
